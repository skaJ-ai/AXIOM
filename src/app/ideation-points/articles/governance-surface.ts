...
const GOVERNANCE_SURFACE_ARTICLES: IdeationPointArticle[] = [
  {
    blocks: [
      {
        paragraphs: [
          ...
          '이 순서를 지키면 초기 제품은 개인 워크벤치로 시작할 수 있고, 이후 shared workspace와 review workflow를 붙여도 설계가 크게 뒤틀리지 않는다. HARP의 갭 분석이 지적한 문제도 바로 이 승격 표면이 아직 얇다는 데 있다.',
        ],
    ...
    sources: [
      ...
      createSource('reference', 'C:/dev/HARP/JARVIS/04_enterprise_gap_analysis.md'),
    ],
    summary:
  ...
  },
  {
    blocks: [
      ...
      {
        paragraphs: [
          '왜 시스템명보다 액션 타입으로 분류하느냐 하면, 연동 대상은 계속 바뀌지만 위험의 종류는 상대적으로 안정적이기 때문이다. SharePoint든 Drive든 결국 import, read, write, export, notify 같은 경계 위에 올라온다.',
    ...
    sources: [
      createSource('draft', 'C:/dev/HARP/JARVIS/07_execution_plan.md'),
      createSource('reference', 'C:/dev/HARP/JARVIS/04_enterprise_gap_analysis.md'),
  ...
  },
  {
    blocks: [
      {
        paragraphs: [
          ...
          'HARP의 갭 분석 문서가 reviewer, approver, publisher 같은 세분 권한을 강조하는 이유도 여기에 있다. AI 문서 제품이 조직 안에서 쓰이려면 생성 능력보다 통제면이 먼저 강해져야 한다.',
        ],
    ...
    sources: [
      createSource('reference', 'C:/dev/HARP/JARVIS/04_enterprise_gap_analysis.md'),
      createSource('reference', 'C:/dev/HARP/JARVIS/05_priority_roadmap.md'),
      createSource('code', 'src/lib/db/schema.ts'),
    ],
    summary:
  ...
  },
  {
    blocks: [
      {
        paragraphs: [
          'Human-in-the-loop는 사람이 매번 다시 쓰게 하는 퇴행이 아니라, 어떤 순간에 최종 확정권을 되돌려 받을지를 분명히 하는 설계다. AXIOM에서 보고서 finalize 이후에만 knowledge pipeline이 도는 구조는 이 원칙을 코드로 표현한 예다.',
      ...
      {
        paragraphs: [
          'HARP 문서들이 review request, comments, approval history를 강조하는 것도 같은 이유다. AI가 초안을 빨리 만들수록 사람의 개입은 앞단 생성이 아니라 뒷단 확정과 검수에 집중되어야 한다.',
  ...
  },
  {
    blocks: [
      {
        paragraphs: [
          'Trust-tier promotion은 모든 문서를 같은 수준의 자산으로 취급하지 않겠다는 선언이다. HARP 로드맵이 제안한 `raw source → draft → final deliverable → promoted asset → verified standard asset` 구간은 생성물의 신뢰와 재사용 권한을 함께 다룬다.',
      ...
      {
        paragraphs: [
          'AXIOM의 현재 상태값인 draft, final, promoted_asset은 이 사다리의 초기 버전이다. 여기에 verified standard asset 같은 상위 계층을 추가하면 조직 공통 자산과 개인 작업 결과를 더 명확히 분리할 수 있다.',
