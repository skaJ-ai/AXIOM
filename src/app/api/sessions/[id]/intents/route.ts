...
async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const intents = await listIntentFragmentsBySession(id, currentUser.workspaceId);
    ...
    const message = error instanceof Error ? error.message : 'Unknown intent query error';
