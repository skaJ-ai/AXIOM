...
import { promoteIntentFragment } from '@/domains/intents/actions';
...
async function POST(
  ...
  try {
    ...
    const isPromoted = await promoteIntentFragment(intentId, currentUser.workspaceId);
    ...
    return NextResponse.json(
      {
        ...
        message: isPromoted ? '의도 조각을 승격했습니다.' : '승격할 의도 조각이 없습니다.',
        status: 200,
      },
      {
        status: 200,
    ...
    const message = error instanceof Error ? error.message : 'Unknown intent promotion error';
