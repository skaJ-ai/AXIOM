...
import { listClustersBySession } from '@/domains/diverge/queries';
...
async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const clusters = await listClustersBySession(id);
    ...
    const message = error instanceof Error ? error.message : 'Unknown clusters error';
